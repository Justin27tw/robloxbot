...
def format_badge_html(g_data, group_type):
    ...
    if group_type == "core": type_icon = "🏴"
    elif group_type == "ally": type_icon = "⚠️"
    else: type_icon = "🎯"
...
def fetch_alert_data(user_id, user_name, relation_type, warning_group_ids, scanned_group_id=None):
    ...
    report = {
        "user_name": user_name, "user_id": user_id, "relation": relation_type,
        "avatar_url": get_user_thumbnail(user_id), 
        "core_groups": [], "ally_groups": [], "scanned_ally_groups": [], "grouped_matches": []
    }
...
def draw_summary_dashboard(alerted_list, total_scanned, title="掃描總結"):
    ...
    col3.metric("🛡️ 安全比例", f"{safe_ratio:.1f} %", delta_color="normal" if safe_ratio == 100 else "off")
    
    if flagged_count > 0:
        df_data = [{
            "頭像": m["avatar_url"], "名稱": m["user_name"], "關聯": m["relation"],
        ...
        if st.button("啟動掃描", type="primary"):
            ...
            if not uid: st.error("無法解析目標。")
                ...
                if friends:
                    ...
                    draw_summary_dashboard(alerted_f, len(friends), "好友圈")
                st.balloons()

    with tab2:
        target_group = st.text_input("目標群組 ID：", key="input_group")
        if st.button("1. 獲取群組結構"):
            roles = get_group_roles(target_group)
            if roles: st.session_state.group_roles_cache[target_group] = sorted(roles, key=lambda x: x.get("rank", 0)); st.success("結構解析成功！")
        
        if target_group in st.session_state.group_roles_cache:
            roles = st.session_state.group_roles_cache[target_group]
            opts = [f"[Rank: {r['rank']}] {r['name']} ({r['memberCount']} 人)" for r in roles]
            s_idx = st.selectbox("起始階層：", range(len(opts)), index=0)
            e_idx = st.selectbox("結束階層：", range(len(opts)), index=len(opts)-1)
            
            if st.button("2. 執行掃描", type="primary"):
                sel_roles = roles[min(s_idx, e_idx) : max(s_idx, e_idx)+1]
                mems = get_members_of_roles(target_group, sel_roles)
                if mems:
                    alerted_m = []
                    bar = st.progress(0)
                    for i, m in enumerate(mems):
                        bar.progress((i+1)/len(mems))
                        a = fetch_alert_data(m["id"], m["name"], f"成員 [{m['rank_name']}]", WARNING_GROUP_IDS, int(target_group))
                        if a: draw_alert_card(a); alerted_m.append(a)
                    draw_summary_dashboard(alerted_m, len(mems), "群組掃描")
                    st.balloons()